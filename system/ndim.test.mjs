...
// ── Projection chips have real authority (2026-08-04 control sweep) ───────────
// The sweep found all three projection chips produced identical frames: studio.js read the mode
// and dropped it before the render. These tests hold the whole path: the modes must reach
// embedTo3D, and must produce genuinely different embeddings for a 4D vertex with w != 0.

import { embedTo3D } from "./lib/render-nd/core/embed3d.mjs";
import { nOrthoplexEdges } from "./lib/render-nd/core/polytopes.mjs";

test("the three projection modes disagree wherever a higher axis is nonzero", () => {
  const v = [0.6, 0.4, 0.3, 0.7];   // a tesseract-ish vertex with w = 0.7
  const persp = embedTo3D(v, 3.0, "perspective");
  const ortho = embedTo3D(v, 3.0, "orthographic");
  const stereo = embedTo3D(v, 3.0, "stereographic");
  const d = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
  assert.ok(d(persp, ortho) > 1e-3, "perspective vs orthographic differ");
  assert.ok(d(persp, stereo) > 1e-3, "perspective vs stereographic differ");
  assert.ok(d(ortho, stereo) > 1e-3, "orthographic vs stereographic differ");
  // Orthographic literally drops the higher axes: the first three coords survive unchanged.
  assert.deepEqual(ortho, [0.6, 0.4, 0.3]);
});

test("for n <= 3 every mode is the identity embedding", () => {
  for (const mode of ["perspective", "orthographic", "stereographic"]) {
    assert.deepEqual(embedTo3D([0.5, -0.2, 0.9], 3.0, mode), [0.5, -0.2, 0.9], mode);
  }
});

test("the volumetric scene carries the mode end to end", () => {
  const at = (mode) => renderSceneVolumetric(
    { kind: "cube", n: 4, t: 0.35, rotation: "all", projectionMode: mode },
    { yaw: 0.4, pitch: 0.2, dist: 3.2 },
    { aspect: 1, scale: 1, focal: 2 },
  );
  const p = at("perspective"), s = at("stereographic");
  assert.equal(p.meta.projection, "perspective3d");
  assert.equal(s.meta.projection, "stereographic3d");
  // Same rotation, same camera: if the vertices land in identical places the mode did nothing.
  let moved = 0;
  for (let i = 0; i < Math.min(p.points.length, s.points.length); i += 1) {
    if (Math.abs(p.points[i].x - s.points[i].x) > 1e-4 || Math.abs(p.points[i].y - s.points[i].y) > 1e-4) moved += 1;
  }
  assert.ok(moved > p.points.length / 2, `only ${moved}/${p.points.length} vertices moved between modes`);
});

test("the 1-orthoplex has its one real edge (it rendered as nothing before)", () => {
  assert.deepEqual(nOrthoplexEdges(1), [[0, 1]]);
  // n >= 2 keeps the antipodal exclusion: 2n vertices, 2n(2n-1)/2 - n edges.
  assert.equal(nOrthoplexEdges(2).length, 4);
  assert.equal(nOrthoplexEdges(3).length, 12);
});
