...
export function respond(message, ctx) {
  ...
  if (ask("colour", "color", "hue", "palette", "tint", "shade")) {
    const cStr = colors.join(", ");
    if (!cStr) return `I'm not reading a strong colour cluster right now — the frame is mostly ${hue}. Fingerprint: ${phash}.`;
    const conText = con !== null
      ...
      : "";
    return `Dominant colours: ${cStr} — it reads as ${hue}-dominant.${conText} Fingerprint: ${phash}.`;
  }
  ...
  if (ask("structure", "detail", "busy", "complex", "texture", "noise", "pattern")) {
    ...
    const mv = motionLine();
    return `Structure (entropy): ${f2(str)} — ${sw}${ew}. Right now it's ${mv}. Source: ${src} at ${w}×${h}.`;
  }
  ...
  if (ask("contrast", "bright", "dark", "light", "tone", "exposure", "luminance")) {
    ...
    const luma = typeof ctx.features?.balance === "number"
      ...
      : "";
    return `Contrast is ${f2(con)} — ${cw}.${luma} It's ${colorLine()}, ${motionLine()}. Hash: ${phash}.`;
  }

  // motion / movement / moving / still / animated
  if (ask("motion", "move", "moving", "still", "static", "animate", "change")) {
    const delta = Math.round(motion * 64);
    if (delta === 0) {
      return `Completely still — zero frame-to-frame change (Δ0/64). The hash is locked at ${phash}. Source: ${src}.`;
    }
    return `I'm reading movement: Δ${delta}/64 since the last measured frame. That's ${motionLine()}. The current hash is ${phash} on a ${colorLine()} frame.`;
  }
  ...
  if (ask("what is", "what's", "about", "describe", "see", "look", "this", "tell me")) {
    ...
    return `I'm looking at a ${w}×${h} ${src} frame — ${colorLine()}${sw}, ${edgeWord(edge)}, ${motionLine()}. Contrast: ${f2(con)}. Fingerprint: ${phash}.`;
  }

  // what can we / try / next / suggest / idea / do / make
  // why / how do you know / trust / prove / honest / evidence — checked BEFORE try/next (avoids "do" collision)
  ...
  if (ask("try", "next", "idea", "make", "could", "suggest", "explore", "experiment", "what can")) {
    // find the weakest axis to suggest improving
    const axes = [
      ["contrast", con],
      ["structure", str],
      ["balance", bal],
  ...
  if (ask("balance", "symmetr", "centre", "center", "compos")) {
    return bal !== null
      ? `Balance (mass distribution around centre): ${f2(bal)}. ${bal > 0.7 ? "Well centred." : bal > 0.45 ? "Slightly off-centre." : "Weight skews to one side."} Context: ${colorLine()}, ${contrastWord(con)} contrast.`
  ...
  // Never empty. Never identical across different ctx values (snapSummary varies by numbers).
  const delta = Math.round(motion * 64);
  const audioNote = audio
    ? ` Audio level: ${audio.level.toFixed(3)}.`
    : "";
  return `I can only speak to what I'm measuring right now — ${snapSummary()}.${audioNote} Ask me about its colour, contrast, structure, motion${audio ? ", audio" : ""}, or what we could try with it. Hash: ${phash}.`;
}
