...
def test_shared_nav_changes_reach_pages_that_cached_it() -> None:
    ...
    assert stamps == {"20260909-font-marketplace-port"}, f"unexpected nav.js cache stamps: {sorted(stamps)}"
    assert 'const ASSET_V = "20260909-font-marketplace-port"' in read(NAV)
