...
(async () => {
  ...
  try {
    ...
    assert.equal(Number(await block.getByLabel('Heading 1 letter spacing', { exact: true }).inputValue()), -0.03);
    assert.equal(Number(await block.getByLabel('Heading 1 line spacing', { exact: true }).inputValue()), 1.45);
    assert.equal(Number(await block.getByLabel('Heading 1 size', { exact: true }).inputValue()), 0.075);
