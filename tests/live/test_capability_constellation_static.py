...
def test_required_live_routes_and_assets_exist() -> None:
    ...
    assert "Built tooling ecosystem" in home
    assert "Evidence board" in home
    assert "Research, pilot, and support routes" in home
...
def test_system_registry_keeps_flywheel_primary_and_private_security_bounded() -> None:
    ...
    assert records["flywheel"]["evidence"][0]["label"] == "Flywheel v0.6.2"
    assert records["flywheel"]["releaseState"] == "stable Python engine v0.6.2; unsigned Windows desktop v0.6.2; clean-machine, mobile, and 0.7.0 acceptance not claimed"
