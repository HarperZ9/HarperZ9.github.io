...
function bubble(role, text, grounds, rederive) {
  ...
  if (rederive) el.querySelector(".recheck-chip").addEventListener("click", () => {
    ...
    rc.innerHTML = `<div class="rc-line">re-read the canvas pixels → hash <span class="rc-num">${again}</span></div>`
      + `<div class="rc-match ${ok ? "ok" : "bad"}">${ok ? "✓ reproduces the witnessed perception, so you didn't have to trust it" : "✗ differs from the witnessed hash"}</div>`;
...
function modelChoice(f) {
  ...
  return { key: "invert", why: "I'll invert it, since sometimes the negative makes the structure I'm reading easier to see" };
}
...
function actuate(key, who) {
  ...
  bubble("model", `${who === "you" ? "You" : "I"} ran ${TRANSFORMS[key].label}, and that ${verb}. `
