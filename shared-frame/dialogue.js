...
const AXIS_MEANING = {
  ...
  coverage: "how much of the frame the structure actually fills, neither sparse nor clumped",
  contrast: "the spread between the field's light and dark, which sets how legible the structure is",
...
// Read everything answerable straight off the current World, with nothing invented.
  ...
  switch (id) {
    case "look":
      return { text: `I'm looking at ${c.title}, a ${c.isField ? "flowing field" : "spray of points"} from the `
          ...
          + `${c.palette.length}-colour palette. We can pull it in any direction from here, so tell me what you'd like to see, or let me try something.`,
    ...
    case "why":
      return { text: `I'm scoring it on a few named qualities and folding them together into one number, the cohesion, ${fmt(c.score)} `
