...
}`),oe=ae.families.flatMap(e=>e.routes.filter(e=>e.primary).map(t=>({...t,family:e.label,primary:!0}))),se=ae.families.map(e=>({label:e.label,routes:e.routes.filter(e=>!e.primary).map(t=>({...t,family:e.label,primary:!1}))})).filter(e=>e.routes.length),ce=ae.externalActions;function le(e){try{let t=new URL(e||`index.html`,`https://harperz9.github.io/`),n=t.pathname.replace(/^\//,``);return n?n.endsWith(`/`)?n+=`index.html`:(n.split(`/`).pop()||``).includes(`.`)||(n+=`.html`):n=`index.html`,n+t.search+t.hash}catch{return``}}function ue(e){let t=le(e),n=t.split(`#`)[0].split(`?`)[0];for(let e of ae.families)if(e.routes.some(e=>{let r=le(e.href);return r===t||r.split(`#`)[0].split(`?`)[0]===n?!0:(e.aliases||[]).some(e=>{let r=le(e);return r===t||r.split(`#`)[0].split(`?`)[0]===n})}))return e.label;for(let e of ae.families){let t=n.replace(/\.html$/,``);if((e.prefixes||[]).some(e=>t.startsWith(e)))return e.label}return``}var T=[{label:`Flywheel`,href:`flywheel.html`,family:`Systems`},{label:`Bulletin`,href:`bulletin.html`,family:`Bulletin`},{label:`Writing`,href:`publications.html`,family:`Research`},{label:`Fonts`,href:`fonts.html`,family:`Fonts`},{label:`Studio`,href:`studio.html`,family:`Studio`},{label:`Hire me`,href:`hire.html`,family:`Work`}],E=[{label:`Explore`,routes:[{label:`All projects`,href:`catalog.html`,family:`Systems`},{label:`Research`,href:`research.html`,family:`Research`},{label:`Frontier Safety`,href:`frontier-safety.html`,family:`Research`},{label:`Gallery`,href:`gallery.html`,family:`Studio`}]},{label:`About & contact`,routes:[{label:`About Zain`,href:`person.html`,family:`Work`},{label:`Resume`,href:`resume.html`,family:`Work`},{label:`Full site index`,href:`index.html#site-index`,family:``}]}],de=JSON.parse(`{
...
`),fe=de.systems;fe.filter(e=>e.placement===`featured`);var pe=de.domains,me=fe.flatMap(e=>e.evidence.map(t=>({...t,systemId:e.id}))).sort((e,t)=>t.date.localeCompare(e.date));function D(e){return fe.find(t=>t.id===e)}var O=`{
